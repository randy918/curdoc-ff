...
const createArticleMasterArrays = function () {
    ...
    for (var i = 0; i < articles.length; i++) {nameArray.push[i](articles.name[i]);
c(articles.name[i]);
    ...
    searchBar.addEventListener(
        ...
        (e) => {
            ...
            for (var i = 0; i < articles.length; i++) {
                var theArticle = articles[i];
                c(theArticle.date);
                c(theArticle.date.includes('the'));

                const colors = ['red', 'green', 'blue']; 
                const result = colors.includes('red'); console.log(result); // true
